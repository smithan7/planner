...
Agent::Agent(ros::NodeHandle nHandle, const int &test_environment_number, const int &test_scenario_number, const int &agent_index, const int &jetson){
	...
	if(this->index == 0){ // condor
		...
		this->planner_update_subscriber_1 = nHandle.subscribe("/distributed_planner/human/agent_plans", 10, &Agent::planner_update_callback, this);
		this->planner_update_subscriber_2 = nHandle.subscribe("/distributed_planner/raven/agent_plans", 10, &Agent::planner_update_callback, this);
		// hear everyone else's status and completed tasks
		this->planner_status_subscriber_1 = nHandle.subscribe("/distributed_planner/human/dist_planner_status", 10, &Agent::planner_status_callback, this);
		this->planner_status_subscriber_2 = nHandle.subscribe("/distributed_planner/raven/dist_planner_status", 10, &Agent::planner_status_callback, this);
	}
	else if(this->index == 1){ // raven
	// hear everyone else's plans
		this->planner_update_subscriber_1 = nHandle.subscribe("/distributed_planner/human/agent_plans", 10, &Agent::planner_update_callback, this);
		this->planner_update_subscriber_2 = nHandle.subscribe("/distributed_planner/condor/agent_plans", 10, &Agent::planner_update_callback, this);
		// hear everyone else's status and completed tasks
		this->planner_status_subscriber_1 = nHandle.subscribe("/distributed_planner/human/dist_planner_status", 10, &Agent::planner_status_callback, this);
		this->planner_status_subscriber_2 = nHandle.subscribe("/distributed_planner/condor/dist_planner_status", 10, &Agent::planner_status_callback, this);
	
	}
	else if(this->index == 2){ // human
	// hear everyone else's plans
		this->planner_update_subscriber_1 = nHandle.subscribe("/distributed_planner/raven/agent_plans", 10, &Agent::planner_update_callback, this);
		this->planner_update_subscriber_2 = nHandle.subscribe("/distributed_planner/condor/agent_plans", 10, &Agent::planner_update_callback, this);
		// hear everyone else's status and completed tasks
		this->planner_status_subscriber_1 = nHandle.subscribe("/distributed_planner/raven/dist_planner_status", 10, &Agent::planner_status_callback, this);
		this->planner_status_subscriber_2 = nHandle.subscribe("/distributed_planner/condor/dist_planner_status", 10, &Agent::planner_status_callback, this);
